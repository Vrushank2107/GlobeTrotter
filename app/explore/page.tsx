        ...
        <main className="pt-20 md:pt-24 pb-24 md:pb-16 px-4 md:px-10 min-h-screen">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-4 md:p-5 mb-6 shadow-xs flex flex-col md:flex-row items-center gap-4">
            ...
            <div className="flex items-center gap-2 md:gap-3 w-full md:w-auto flex-wrap">
              <div className="flex items-center gap-1 md:gap-1.5 flex-1 md:flex-initial min-w-[120px]">
              ...
              >
                ...
                >
                  ...
                  <div className="absolute top-3 md:top-4 right-3 md:right-4">
                    <span className="bg-white/90 backdrop-blur-md text-slate-900 text-[10px] md:text-xs font-bold px-2 md:px-3 py-1 rounded-full flex items-center gap-1 shadow-xs">
                ...
                <div className="p-4 md:p-6 flex flex-col flex-1">
                  <div className="flex justify-between items-start mb-1">
                    <h3 className="font-bold text-lg md:text-xl text-slate-900">{dest.name}</h3>
                    <span className="text-[10px] md:text-xs font-semibold text-slate-500">{dest.country}</span>
                  </div>

                  <p className="text-[10px] md:text-xs text-slate-500 leading-relaxed mb-3 md:mb-4 line-clamp-2">{dest.description}</p>

                  <div className="flex flex-wrap gap-1 md:gap-1.5 mb-4 md:mb-6">
                    {dest.tags.map((t) => (
                      <span
                        key={t}
                        className="bg-slate-100 text-slate-600 text-[10px] font-semibold px-2 md:px-2.5 py-0.5 rounded-full"
                      >
                        {t}
                      </span>
                    ))}
                  ...
                  <div className="mt-auto pt-3 md:pt-4 border-t border-slate-100 flex flex-col md:flex-row items-start md:items-center justify-between gap-3 md:gap-2">
                    <div>
                    ...
                    <div className="flex items-center gap-2 w-full md:w-auto">
                      <button
                        ...
                        className="flex-1 md:flex-initial bg-slate-100 text-slate-800 hover:bg-slate-200 font-semibold px-3 py-2 rounded-full text-[10px] md:text-xs transition-all cursor-pointer"
                      >
                        + Add to Trip
                      </button>
                      <Link
                        href="/trips/new"
                        className="flex-1 md:flex-initial bg-sky-50 text-sky-700 hover:bg-sky-100 font-semibold px-3.5 py-2 rounded-full text-[10px] md:text-xs transition-all inline-flex items-center gap-1 justify-center"
                      >
                        <span>Plan</span>
            ...
            <div className="fixed inset-0 bg-slate-950/50 backdrop-blur-xs z-50 flex items-center justify-center p-2 md:p-4">
              <div className="bg-white rounded-2xl md:rounded-3xl max-w-md w-full p-4 md:p-8 shadow-2xl border border-slate-200 animate-fade-in mx-2 md:mx-0">
                <div className="flex justify-between items-center mb-4">
                  <h3 className="text-base md:text-lg font-bold text-slate-900">Add {selectedDestForTrip.name} to Trip</h3>
                  <button onClick={() => setSelectedDestForTrip(null)} className="text-slate-400 font-bold text-lg md:text-xl">
                    ✕
                  </button>
                </div>
                <p className="text-[10px] md:text-xs text-slate-500 mb-6">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <select
                      ...
                      className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 md:px-4 py-2 md:py-3 text-[10px] md:text-xs font-medium outline-none focus:border-sky-500"
                    >
                  ...
                  <div className="flex justify-end gap-2 md:gap-3 pt-4 border-t border-slate-100">
                    ...
                    <button
                      ...
                      className="bg-slate-900 hover:bg-slate-800 disabled:opacity-50 text-white font-bold px-4 md:px-6 py-2 md:py-2.5 rounded-full text-[10px] md:text-xs shadow-md inline-flex items-center gap-1.5 md:gap-2 cursor-pointer disabled:cursor-not-allowed"
                    >
                      {isAddingDest ? (
                        <>
