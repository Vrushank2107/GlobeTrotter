          ...
          >
            <div className="relative z-10 flex flex-col md:flex-row md:items-end justify-between w-full gap-4 text-white">
              <div>
                ...
                <h1 className="text-2xl md:text-4xl lg:text-5xl font-bold mb-2">{trip.title}</h1>
                <p className="text-xs md:text-sm text-slate-300 flex items-center gap-2">
                  <Calendar className="w-3.5 h-3.5 md:w-4 md:h-4 text-sky-400" /> {trip.startDate} to {trip.endDate} ({daysCount} Days)
                </p>
              </div>
              ...
              <div className="flex items-center gap-2 md:gap-3">
                <button
                  ...
                  className="bg-white/10 hover:bg-white/20 backdrop-blur-md text-white text-[10px] md:text-xs font-semibold px-3 md:px-4 py-2 md:py-2.5 rounded-full transition-all flex items-center gap-1.5 md:gap-2 cursor-pointer border border-white/20"
                >
                ...
                <Link
                  ...
                  className="bg-sky-500 text-slate-950 hover:bg-sky-400 text-[10px] md:text-xs font-bold px-4 md:px-6 py-2 md:py-2.5 rounded-full transition-all flex items-center gap-1.5 md:gap-2 shadow-lg"
                >
          ...
          <div className="px-4 md:px-10">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 md:gap-6 mb-6 md:mb-8">
              <div className="bg-white border border-slate-200 rounded-2xl p-4 md:p-5 shadow-xs">
                <span className="text-slate-400 font-medium text-[10px] md:text-xs uppercase tracking-wider block mb-1">
                  Destinations
                </span>
                <span className="text-lg md:text-xl font-bold text-slate-900 truncate block">
              ...
              <div className="bg-white border border-slate-200 rounded-2xl p-4 md:p-5 shadow-xs">
                ...
                <span className="text-lg md:text-xl font-bold text-slate-900">{trip.activities.length} Events</span>
              </div>

              <div className="bg-white border border-slate-200 rounded-2xl p-4 md:p-5 shadow-xs flex flex-col justify-between">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    className="text-sky-600 hover:text-sky-700 font-semibold text-[10px] md:text-xs flex items-center gap-1 cursor-pointer"
                  >
                    <Edit3 className="w-3 h-3 md:w-3.5 md:h-3.5" /> Edit
                  </button>
                </div>
                <span className="text-lg md:text-xl font-bold text-slate-900">₹{trip.totalBudget.toLocaleString()}</span>
              </div>
              ...
              <div className="bg-white border border-slate-200 rounded-2xl p-4 md:p-5 shadow-xs">
                ...
                <span className={`text-lg md:text-xl font-bold ${remainingBudget < 0 ? 'text-red-600' : 'text-emerald-600'}`}>
            ...
            {/* Route Sequence Card */}
            <div className="bg-white rounded-3xl border border-slate-200 p-4 md:p-6 mb-6 md:mb-8 shadow-xs">
              <h2 className="text-base md:text-lg font-bold text-slate-900 mb-4">Multi-City Sequence Route</h2>
              <div className="flex items-center gap-3 md:gap-4 overflow-x-auto pb-2">
                {trip.destinations.map((dest, i) => (
                  <React.Fragment key={dest.id}>
                    <div className="flex items-center gap-2 md:gap-3 bg-slate-50 border border-slate-200 rounded-2xl p-3 md:p-3 pr-4 md:pr-5 shrink-0">
                      <img
                        src={dest.image}
                        alt={dest.cityName}
                      ...
                      />
                      <div>
                        <h4 className="font-bold text-xs md:text-sm text-slate-900">{dest.cityName}</h4>
                        <p className="text-[10px] md:text-xs text-slate-500">{dest.country}</p>
                      </div>
                    </div>

                    {i < trip.destinations.length - 1 && (
                      <ArrowRight className="w-4 h-4 md:w-5 md:h-5 text-sky-500 shrink-0" />
                    )}
                  </React.Fragment>
                ))}
              </div>
